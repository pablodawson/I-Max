...
import torch
from pipeline_flux_kontext_imax import FluxKontextPipeline
...
pipe = FluxKontextPipeline.from_pretrained(bfl_repo, transformer=None, torch_dtype=torch.bfloat16)
...
def generate_image(prompt, width, height, edit_image, seed, ntk_factor, num_inference_steps1, num_inference_steps2, guidance_scale1, guidance_scale2, time_shift_1, time_shift_2, dwt_level, guidance_schedule):
    w, h = edit_image.size
    image_size = f"{w}x{h}"
...
demo = gr.Interface(
    ...
    inputs=[
        ...
        gr.Slider(label="Width", value=1024, step=1, minimum=512, maximum=4096),
        gr.Slider(label="Height", value=1024, step=1, minimum=512, maximum=4096),
    ...
    description="Generate images with the I-Max + Flux.1 Kontext dev Pipeline."
)
