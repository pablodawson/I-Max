...
from transformer_flux_new import FluxTransformer2DModel
...
class FluxKontextPipeline(
    ...
    def __call__(
        ...
        ####
        num_inference_steps1: int = 30,
        num_inference_steps2: int = 20,
        guidance_scale1: float = 3.5,
        guidance_scale2: float = 7.0,
        ntk_factor: float = 10.0,
        time_shift_1: float = 3.0,
        time_shift_2: float = 6.0,
        ...
        Args:
            ...
            num_inference_steps1 (`int`, defaults to 30):
                The number of denoising steps for the native resolution phase.
            num_inference_steps2 (`int`, defaults to 20):
                The number of denoising steps for the extrapolated resolution phase.
            guidance_scale1 (`float`, defaults to 3.5):
                Guidance scale for the native resolution phase.
            guidance_scale2 (`float`, defaults to 7.0):
                Guidance scale for the extrapolated resolution phase.
            ntk_factor (`float`, defaults to 10.0):
                NTK scaling factor for attention mechanism in the second phase.
            time_shift_1 (`float`, defaults to 3.0):
                Time shift parameter for the first phase scheduler.
            time_shift_2 (`float`, defaults to 6.0):
                Time shift parameter for the second phase scheduler.
            proportional_attention (`bool`, defaults to True):
                Whether to use proportional attention in the second phase.
            text_duplication (`bool`, defaults to True):
                Whether to use text duplication in the second phase.
            swin_pachify (`bool`, defaults to True):
                Whether to use swin-style patchification in the second phase.
            dwt_level (`int`, defaults to 1):
                DWT decomposition level for frequency component separation.
            guidance_schedule (`str`, defaults to "cosine_shift"):
                Guidance schedule type. Options: "disable", "cosine_decay", "cosine_shift", "constant".
        ...
        scale_factor = math.sqrt((height * width) / native_resolution ** 2)
        ...
        self._guidance_scale = guidance_scale1
        ...
        latents, image_latents, latent_ids, image_ids = self.prepare_latents(
            ...
            height // int(scale_factor + 0.5),
            width // int(scale_factor + 0.5),
            prompt_embeds.dtype,
            device,
            generator,
            latents,
        ...
        timesteps, num_inference_steps1 = retrieve_timesteps(
            self.scheduler,
            num_inference_steps1,
            device,
            timesteps,
            sigmas,
            mu=mu,
        )
        num_warmup_steps = max(len(timesteps) - num_inference_steps1 * self.scheduler.order, 0)
        self._num_timesteps = len(timesteps)

        # handle guidance
        if self.transformer.config.guidance_embeds:
            guidance = torch.full([1], guidance_scale1, device=device, dtype=torch.float32)
        ...
        # 6.1 Denoising loop (Native Resolution) ###########################################################################
        # We set the index here to remove DtoH sync, helpful especially during compilation.
        # Check out more details here: https://github.com/huggingface/diffusers/pull/11696
        self.scheduler.set_begin_index(0)
        with self.progress_bar(total=num_inference_steps1) as progress_bar:
        ...
        if image_ids is not None:
            latent_ids = torch.cat([latent_image_ids_new, image_ids], dim=0)  # dim 0 is sequence dimension
        else:
            latent_ids = latent_image_ids_new
        ...
        self.scheduler.config.shift = time_shift_2
        timesteps_extra, num_inference_steps2 = retrieve_timesteps(
            self.scheduler,
        ...
        # 6.4 Denoising loop (Extrapolated Resolution) ###########################################################################
        with self.progress_bar(total=num_inference_steps2) as progress_bar:
            for i, t in enumerate(timesteps_extra):
                ...
                # handle guidance
                if self.transformer.config.guidance_embeds:
                    guidance = torch.tensor([guidance_scale2], device=device)
                    guidance = guidance.expand(latents.shape[0])
                else:
                    guidance = None
                ...
                # Flow Projection
                fp_v = -(latents_guidance - latents) / (t / 1000 + 1e-6)
                ...
                elif guidance_schedule == "cosine_decay":
                    noise_pred = noise_pred + cosine_factor * (fp_v_low - noise_pred_low)
                elif guidance_schedule == "cosine_shift":
                    noise_pred = noise_pred + cosine_factor * (fp_v - noise_pred) + (1 - cosine_factor) * (fp_v_low - noise_pred_low)
                ...
                # call the callback, if provided
                if i == len(timesteps_extra) - 1 or ((i + 1) > num_warmup_steps and (i + 1) % self.scheduler.order == 0):
                    progress_bar.update()
        ...
        if output_type == "latent":
            ...
            image_guidance = self.image_processor.postprocess(image_guidance, output_type=output_type)

        # Offload all models
        self.maybe_free_model_hooks()

        if not return_dict:
            return image + image_guidance
            return (image,)
